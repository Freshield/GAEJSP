 ...
 */
 public class HelloServlet extends javax.servlet.http.HttpServlet 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
	    throws ServletException, IOException {
		long timeNow = System.currentTimeMillis()-50000;
		//store the info get
        ...
        try {
            ...
            else if((rname.equals("flask")) || (rname.equals("flake"))){
            	ResultSet rs = stmt.executeQuery(sql);// executeQuery会返回结果的集合，否则返回空值
                String temp = "";
                System.out.println("1");
                    while (rs.next() && (amount <5)) {
                    	
                    	temp = rs.getString("cpu");
                    	if(temp.equals("")){
                    		temp = "0";
                    	}
                    	double buf = Double.parseDouble(temp);
                    	...
                    	System.out.println("4294967296- Double.parseDouble(temp)"+(4294967296l- Double.parseDouble(temp))/4294967296l);
                    	buf = ((4294967296l- Double.parseDouble(temp))*100)/4294967296l;
                    	System.out.println("buf"+buf);
                    	result = String.format("%.3f",buf);
                    	System.out.println("result"+result);
                    	mem.add(result);
